...
let kind_to_tag, tag_to_style, register_ressource =
  ...
  let kind_to_tag = function
    | LibIndex.OpenType | LibIndex.Type -> "Type"
    | Value -> "Value"
    | Exception -> "Exception"
    | Field _ -> "Field"
    | Variant _ -> "Variant"
  ...
  let register_ressource res s default =
    ...
    | Some res ->
      ...
      if res_style = LTerm_style.none then Hashtbl.add h s default
      else Hashtbl.add h s res_style
    | None -> Hashtbl.add h s default
  in
...
let pp_with_style to_style =
 fun style fstr fmt ->
  ...
  pp_open_tag fmt tag;
  Format.kfprintf (fun fmt -> pp_close_tag fmt ()) fmt fstr

let colorise opts =
  if not opts.IndexOptions.color then LibIndex.Format.no_color
  else
    let f kind fstr fmt = pp_with_style kind_to_tag kind fstr fmt in
    { LibIndex.Format.f }

(** Format the complete answer and return a styled text. *)
let sprint_answer ?(extra_info = false) cols colorise id =
  let get_content, fmt =
    LTerm_text.make_formatter ~read_color:tag_to_style ()
  in
  ...
  begin
    ...
    | { LibIndex.ty = None; _ }
    | { LibIndex.kind =
          ( LibIndex.Module | LibIndex.ModuleType | LibIndex.Class
          | LibIndex.ClassType )
      ; _
      } ->
      ()
  ...
  print "@]";
  if extra_info && Lazy.force id.LibIndex.doc <> None then begin
    print "@\n    ";
...
let load_bindings () =
  ...
  bind
    ...
    [ edit Prev_char ];
  bind
    [ { control = false; meta = true; shift = false; code = Backspace } ]
    [ edit Delete_prev_word ];

  bind
    [ { control = false; meta = true; shift = false; code = Home } ]
    [ Complete_bar_first ];
  bind
    [ { control = false; meta = true; shift = false; code = End } ]
    [ Complete_bar_last ];
  bind
    [ { control = false; meta = false; shift = false; code = Up } ]
    [ Complete_bar_prev ];
  bind
    [ { control = true; meta = false; shift = false; code = ~/'p' } ]
    [ Complete_bar_prev ];
  bind
    [ { control = false; meta = false; shift = false; code = Down } ]
    [ Complete_bar_next ];
  bind
    [ { control = true; meta = false; shift = false; code = ~/'n' } ]
    [ Complete_bar_next ];

  bind
    [ { control = false; meta = true; shift = false; code = Up } ]
    [ Complete_bar_prev ];
  bind
    [ { control = false; meta = true; shift = false; code = Down } ]
    [ Complete_bar_next ];
  bind
    [ { control = false; meta = true; shift = false; code = Right } ]
    [ Complete_bar ];
...
class virtual line_editor =
  object (self)
    ...
    method! update_resources =
      let rc = self#resource_class
      and resources = self#resources in
          ...
          let res =
            ...
            | None ->
              Bindings.resolver
                ...
                ; Bindings.pack
                    (List.map (fun x -> LTerm_read_line.Edit x))
                    !LTerm_edit.bindings
                ]
          in
            ...
            if resolver = None then
              ...
              | { control = false; meta = false; shift = false; code = Char ch }
                ...
                Zed_macro.add self#macro
                  (Edit
                     (LTerm_edit.Zed
                        (Zed_edit.Insert (Zed_char.unsafe_of_uChar ch)) ) );
    ...
    method! send_action =
      function Edit (Zed Newline) -> () | action -> super#send_action action
    ...
    method! draw ctx _focused =
      ...
      let cursor_offset =
        Zed_cursor.get_position (Zed_edit.cursor self#context)
      in
      let cursor_line = Zed_lines.line_index line_set cursor_offset in
      let cursor_column =
        cursor_offset - Zed_lines.line_start line_set cursor_line
      in
      ...
      let rec draw_line row col zip =
        if Zed_rope.Zip.at_eos zip then draw_eoi (row + 1)
        else
      ...
      and skip_eol row zip =
        if Zed_rope.Zip.at_eos zip then draw_eoi (row + 1)
          ...
          let char, zip = Zed_rope.Zip.next zip in
          if char = newline then begin_line row zip else skip_eol row zip
      and skip_bol row zip remaining =
        if remaining = 0 then draw_line row 0 zip
        else if Zed_rope.Zip.at_eos zip then draw_eoi (row + 1)
        else
          let char, zip = Zed_rope.Zip.next zip in
          if char = newline then begin
            let row = row + 1 in
          ...
          else skip_bol row zip (remaining - 1)
      and begin_line row zip =
        if Zed_rope.Zip.at_eos zip then draw_eoi row
        else if shift <> 0 then begin
          skip_bol row zip shift
        end
        else draw_line row 0 zip
      and draw_eoi _row = () in
      ...
      if Zed_edit.get_selection self#edit then begin
        ...
        let line_a, column_a, line_b, column_b =
          ...
          else (cursor_line, cursor_column, sel_line, sel_column)
        in
        let line_a, column_a =
          if line_a < start_line then (start_line, 0) else (line_a, column_a)
        in
        let line_b, column_b =
          if line_b >= start_line + size.rows then
            (start_line + size.rows - 1, size.cols - 1)
          else (line_b, column_b)
        in
        if line_a < start_line + size.rows && line_b >= start_line then begin
          let line_a = line_a - start_line
          and line_b = line_b - start_line in
          let column_a = column_a
          and column_b = column_b in
...
let filter_completion l =
  ...
  let aux (s, l) ({ LibIndex.name; kind; _ } as h) =
    match kind with
    | Module -> begin
      match Re.split re_double_undescore name with
...
let sort_completion l =
  let cmp { LibIndex.file = f1; loc_impl = l1; _ }
    ...
    if i <> 0 then i
    else compare (Lazy.force l1).loc_start (Lazy.force l2).loc_start
  in
  List.sort cmp l
...
class completion_box options exit =
  let completion_info, set_completion_info =
    S.create ~eq ([] : LibIndex.info list)
  in
  ...
  object (self)
    ...
    initializer
      self#on_event (function
        | LTerm_event.Key
            { control = false; meta = true; shift = false; code = Left } ->
    ...
    method set_module_state b = module_state <- b
    ...
    method! completion =
      ...
      let completions =
        ...
        List.map
          (fun x ->
            ( Zed_string.of_utf8 (LibIndex.Print.path ~short:true x)
            , Zed_string.of_utf8 (suffix x) ) )
          response
      in
    ...
    method! is_valid_char x =
      try
        match Uchar.to_char x with
        | 'a' .. 'z'
        | 'A' .. 'Z'
        | '0' .. '9'
        | '_' | '\'' | '#' | '!' | '$' | '%' | '&' | '*' | '+' | '-' | '.' | '/'
...
(** The show box shows the result of a research.
    ...
    [content] is a list zipper positioned at the focused element. Left and right
    lists are elements before and after the focus.
...
class show_box color =
  object (self)
    ...
    method toogle_extra_info = extra_info <- not extra_info
    ...
    method! draw ctx _focused =
      ...
      | left, focus :: right -> begin
        ...
        let rec format (dir : [ `L | `R ]) left right size_l size_r format_l
          format_r =
          ...
          | `L, info :: t, _ | `R, _, info :: t ->
            ...
            if dir = `L then
              ...
              format dir left t size_l (size + size_r) format_l
                ((text, size) :: format_r)
        in
        ...
        let start =
          max 0
            (min size_left (max default_pos (rows - right_size - size_focus)))
        in
        ...
        let doc_hint_char =
          match Lazy.force focus.LibIndex.doc with
          | Some _ -> if extra_info then '-' else '+'
          | None -> ' '
        in
        LTerm_draw.draw_char ctx start 0
        @@ Zed_char.unsafe_of_char doc_hint_char;
...
let pp_kinds fmt options =
  let pp_kind fmt (c, hash, b) =
    if b then (
      ...
      pp_close_tag fmt () )
    else pp_with_style (fun x -> x) "Disabled" "%s" fmt c
  in
...
class frame_info options =
  object
    ...
    method! draw ctx focused =
      ...
      if width > len + 2 then LTerm_draw.draw_styled ctx 0 (width - len - 1) s
  end
...
let event_handler (cbox : #completion_box) (sbox : #show_box) options show_help
  ...
  | LTerm_event.Key
    ...
    let new_state =
      if cbox#module_state then fil.t && fil.e && fil.c && fil.k && fil.v
      else fil.t || fil.e || fil.c || fil.k || fil.v
    in
    cbox#set_module_state new_state;
    let new_b = not new_state in
    options.filter <-
      { fil with t = new_b; e = new_b; c = new_b; k = new_b; v = new_b };
    cbox#completion;
    sbox#queue_draw;
    true
  | LTerm_event.Key
  ...
  | LTerm_event.Key
    ...
    for _i = 0 to max 1 k do
      cbox#send_action Complete_bar_prev
    done;
    true
  | LTerm_event.Key
    ...
    for _i = 0 to max 1 k do
      cbox#send_action Complete_bar_next
    done;
    true
  | LTerm_event.Key
...
class help_label options =
  let text =
    LTerm_text.styprintf ~read_color:tag_to_style help_content pp_kinds options
  in
  let h = size text in
  object
    ...
    method! draw ctx _focused = LTerm_draw.draw_styled ctx 0 0 text
  end
...
let main options =
  let do_run, push_layer, pop_layer, exit =
    LTerm_widget.prepare_simple_run ()
  in
...
let run options () =
  Lwt_main.run (load_style () >>= load_bindings >>= fun () -> main options)
...
let main_term =
  ...
  let term =
    Term.(
      const run
      $ IndexOptions.common_opts
          ~default_filter:[ `T; `V; `E; `C; `M; `S; `K ]
          ()
      $ const () )
  in
  let info =
    Cmd.info "ocp-browser" ~version:Ocp_browser_version.version ~doc ~man
  in
  Cmd.v info term
