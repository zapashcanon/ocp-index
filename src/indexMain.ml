...
let default_info =
  let man =
    [ `S "DESCRIPTION"
    ; `P
        "ocp-index is a simple and light-weight documentation extractor for \
         OCaml, for command-line or integrated use (e.g. for completion). It \
         gathers information from .cmi (like ocamlbrowser) and .cmt/cmti \
...
let format_man =
  let formats =
    ...
    ; ( "%q"
      , "The qualified ident, e.g. \"List.map\", or possibly \"map\" if the \
         ident was accessed through an \"open\"" )
    ; ("%p", "The full path of the ident, e.g. \"List.map\"")
    ; ( "%k"
      , "The kind of the ident, one of \"type\", \"val\", \"exception\", \
    ...
    ; ( "%e"
      , "The enclosing type definition, for field records, variants and methods"
      )
    ; ("%%", "A single '%' character")
    ]
  in
  [ `S "FORMAT STRINGS"
  ; `P
      "Format strings are arbitrary strings that will be printed for every \
...
let man =
  [ `S "COPYRIGHT"
  ; `P
      "Ocp-index is written by Louis Gesbert <louis.gesbert@ocamlpro.com>, \
       copyright OCamlPro 2013-2014, distributed under the terms of the LGPL \
       v3 with linking exception. Full source available at \
       $(i,https://github.com/OCamlPro/ocp-index)"
  ; `S "BUGS"
...
let complete_cmd =
  let man =
    [ `S "DESCRIPTION"
    ; `P
        "Searches for OCaml identifiers matching a given prefix in $(b,.cmi), \
  ...
  let separate : bool Term.t =
    let doc =
      "When using a format string, format each escape independently rather \
       than as a whole. Useful when generating (elisp) code."
    in
  ...
  let print_compl opts sexpr format separate query =
    ...
    let results =
      LibIndex.complete opts.IndexOptions.lib_info
        ~filter:(IndexOptions.filter opts) query
    in
    if sexpr then (
      if format <> None then
      ...
      List.iter
        (fun info ->
          ...
          Format.fprintf fmt "(@[<v 2>\"%a\""
            !(LibIndex.Format.path ~short:true)
            info;
          Format.fprintf fmt "@ (:path . \"%a\")"
            !(LibIndex.Format.path ~short:false)
            info;
          Format.fprintf fmt "@ (:type . %S)" (LibIndex.Print.ty info);
          Format.fprintf fmt "@ (:kind . \"%a\")" !LibIndex.Format.kind info;
          if Lazy.force info.LibIndex.doc <> None then
            Format.fprintf fmt "@ (:doc . %S)" (LibIndex.Print.doc info);
          Format.fprintf fmt "@]@ )@ " )
        results;
      Format.fprintf fmt "@]@ )@." )
    else
      ...
      let print fmt i =
        ...
        | Some fstring ->
          LibIndex.Format.format ?root:opts.IndexOptions.project_root ~separate
...
let type_cmd =
  let man =
    [ `S "DESCRIPTION"
    ; `P
        "Prints the type(s) of an OCaml ident as found in $(b,.cmi), $(b,.cmt) \
         and $(b,.cmti) files. Unless specified, the search path includes the \
         OCaml libraries prefix and the current project build directory. This \
         is equivalent to `ocp-index print $(docv) \"%t\"'"
    ]
    @ man
  in
  ...
  Cmd.v (Cmd.info "type" ~doc ~man) Term.(const print_ty $ common_opts $ t)

let locate_cmd =
  let man =
    [ `S "DESCRIPTION"
    ; `P
        "Prints the source location(s) of an OCaml ident as found in \
         $(b,.cmt), or $(b,.cmti) files. Unless specified, the search path \
         includes the OCaml libraries prefix and the current project build \
         directory."
    ]
    @ man
  ...
  let interface : bool Term.t =
    let doc =
      "Lookup the interface instead of the implementation, if it exists"
    in
...
let print_cmd =
  let man =
    [ `S "DESCRIPTION"
    ; `P
        "Searches for an OCaml identifier in $(b,.cmi), $(b,.cmt) and \
         $(b,.cmti) files. Unless specified, the search path includes the \
         OCaml libraries prefix and the current project build directory."
    ]
    @ format_man @ man
  in
  let query =
  ...
  let separate : bool Term.t =
    let doc =
      "Format each escape in the format string independently rather than as a \
       whole. Useful when generating (elisp) code."
    in
    Arg.(value & flag & info [ "separate" ] ~doc)
  in
...
let () =
  try exit @@ Cmd.eval ~catch:false full_cmd with
  | LibIndex.Bad_format f ->
    Printf.eprintf
      "\r\027[K[ERROR] %S can't be read.\n\
       It's likely that it belongs to a version of OCaml different from the \
       one ocp-index was compiled against.\n"
      f;
    exit 4
  | e ->
