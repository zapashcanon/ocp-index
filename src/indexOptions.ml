...
let common_opts ?(default_filter = default_filter) () : t Term.t =
  let ocamllib : string list Term.t =
    let no_stdlib : bool Term.t =
      let doc =
        "Don't include the OCaml standard library directory (`ocamlc -where`) \
         in lookups."
      in
    ...
    let set_default no_stdlib no_opamlib includes =
      ...
      let paths =
        if no_opamlib then paths
        else
          try cmd_input_line "opam config var lib" :: paths
          with Failure _ -> paths
      in
      if no_stdlib then paths
      else try cmd_input_line "ocamlc -where" :: paths with Failure _ -> paths
    in
  ...
  let color : bool Term.t =
    let arg =
      let choices =
        Arg.enum [ ("always", `Always); ("never", `Never); ("auto", `Auto) ]
      in
      let doc =
        "Colorize the output. $(docv) is either `always', `never' or `auto'."
      in
      Arg.(
        last & opt_all choices [ `Auto ]
        & info [ "c"; "color" ] ~docv:"WHEN" ~doc )
    in
  ...
  let open_modules : (string list list * string list list) Term.t =
    let arg_open =
      let doc =
        "Consider the given (comma-separated list of) modules are opened for \
         lookup."
      in
      Arg.(
        value
        & opt_all (list ~sep:',' (list ~sep:'.' string)) []
        & info [ "O"; "open" ] ~docv:"MODULES" ~doc )
    in
    let arg_full_open =
      let doc =
        "Like `--open', but if the .cmt is available, load even the elements \
         that would be restricted by the interface. Useful for the current \
         file."
      in
      Arg.(
        value
    ...
    in
    Term.(
      const (fun o fo -> (List.flatten o, List.flatten fo))
  ...
  let filter : filter_kind Term.t =
            ...
            ~doc:
              (Printf.sprintf
                 "Kinds of elements to show in answers: $(docv) is a \
                  ...
                  methods, `$(i,exceptions)', `$(i,constructs)' (record fields \
                  and sum type constructors), `$(i,modules)' and classes, \
                  `$(i,sigs)' (module and class types), `$(i,keywords)'. The \
                  default is $(b,%s)"
                 (filter_to_string default_filter) )
            ~docv:"LIST" )
    in
    let hide =
      Arg.(
        ...
        & info [ "h"; "hide" ]
            ~doc:
              "kinds of elements not to show in answers: $(docv) is a \
    ...
    Term.(
      const (fun show hide ->
        let f key =
          List.mem key show
          || (List.mem key default_filter && not (List.mem key hide))
        in
        let t, v, e, c, m, s, k = (f `T, f `V, f `E, f `C, f `M, f `S, f `K) in
        { t; v; e; c; m; s; k } )
      $ show $ hide )
  in
  ...
  let context : (bool * string option * int option * int option) option Term.t =
    let doc =
      "Will analyse the context at given FILE[:LINE[,COL]] to give appropriate \
       answers w.r.t open modules, etc. To read from stdin, specify just `:', \
       or alternatively `FILENAME:-' to still specify a filename which will be \
       used to detect e.g. the owning library scope."
    in
    let filepos_converter =
      ( (fun str ->
          match fst Arg.(pair ~sep:':' string string) str with
          ...
          | `Ok (file, "-") -> (
            ...
            | `Ok file -> `Ok (true, Some file, None, None)
            | `Error e -> `Error e )
          | `Ok (file, pos) -> (
            match
              ((fst Arg.non_dir_file) file, (fst (Arg.list Arg.int)) pos)
            with
            | `Ok file, `Ok [ line; col ] ->
              `Ok (false, Some file, Some line, Some col)
            | `Ok file, `Ok [ line ] -> `Ok (false, Some file, Some line, None)
            | `Error e, _ | _, `Error e -> `Error e
            | _ ->
              `Error
                (Printf.sprintf
                   "Wrong file position %S, should be <line> or <line>,<col>"
                   pos ) )
          | `Error _ -> (
  ...
  let lib_info ocamllib (_root, build) (opens, full_opens) context =
      ...
      | Some (use_stdin, file, line, column) ->
        ...
        let info =
          List.fold_left
            ...
            info
            (context_opens @ IndexScope.to_list scope)
        in
        info
    in
    let info =
    ...
    let info =
      List.fold_left
        (LibIndex.fully_open_module ~qualify:true ~cleanup_path:true)
        info full_opens
    in
    info
  in
  Term.(
    const (fun ocamllib project_dirs opens color filter context ->
      { lib_info = lib_info ocamllib project_dirs opens context
      ; color
      ; filter
      ; project_root = fst project_dirs
      } )
