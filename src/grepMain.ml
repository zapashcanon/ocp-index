...
module Grep : sig
  ...
  let path_of_curpath = function
    | `Dot :: _ -> []
    | cp ->
      List.rev_map (function `Uid s | `Lid s -> s | `Dot -> assert false) cp

  let rec list_rm_pfx pfx l =
    match (pfx, l) with
  ...
  let ident path f ch =
    ...
    let f (curpath, lookfor, last_scope, acc) scope tok pos =
      ...
      then (curpath, lookfor, scope, pos :: acc)
    ...
    let _, _, _, matches =
      IndexScope.fold f
        ([], [], IndexScope.empty, [])
        ~init:
          [ IndexScope.Open [ "Pervasives" ]
          ; IndexScope.Open [ "Stdlib" ]
          ; IndexScope.Open modpath
          ]
        ch
    in
    matches
  ...
  let strings_re re _f ch =
    let match_re s =
      let rec aux acc pos =
        let ofs =
          try Some (Re.Group.offset (Re.exec ~pos re s) 0)
          with Not_found -> None
        in
        match ofs with
        | None -> acc
...
module Args = struct
  ...
  let pattern =
    let doc =
      "Fully qualified ident to search for (eg. `List.map', `Set.Make.add', \
       ...), or string with option `-s'."
    in
    Arg.(required & pos 0 (some string) None & info [] ~docv:"ID" ~doc)

  let files =
    let doc =
      "Files or directories to search into. By default, searches for project \
       root"
    in
    ...
    let get_files = function
      | [] ->
        let dir =
          match IndexMisc.project_root () with
          | Some d, _ -> d
...
let rec spaceleft str i =
  ...
  if i >= String.length str || notspace str.[i] then i else spaceleft str (i + 1)
...
let lines_of_file ch matches =
  let rec seek l0 col0 ((curline, txt, n, len) as cur) matches acc =
    ...
    if overflow < 0 then
      ...
      let txt = input_line ch
      and curline = curline + 1 in
      let n =
  ...
  let rec aux curline = function
    ...
    | (l, col, len) :: matches when l <= curline + 1 ->
      ...
      and curline = curline + 1 in
      assert (l = curline);
      let matches, curline, rets =
        seek curline col (curline, txt, col, len) matches []
      in
    ...
    if not color then fun (l, txt, _) ->
      ...
      let ofs =
        List.fold_right
          ...
          toks 0
      in
      if ofs < strlen then print_string (String.sub txt ofs (strlen - ofs));
      print_newline ()
  in
...
let grep pattern files color strings regexp =
  if strings || regexp then
    let re =
      Re.Posix.compile (if regexp then Re.Posix.re pattern else Re.str pattern)
    in
    List.fold_left
...
let () =
  ...
  let doc =
    "Locates instances of a given OCaml ident in source files, handling \
     (local) opens, module, etc."
  in
  let man =
    [ `S "BUGS"
    ; `P
        "Current version doesn't handle shadowing and different kinds of \
         idents, therefore you can get false positive if a type and a value \
         have the name."
    ; `P
        "Field records don't currently respect the distributive `{Module.' \
  ...
  match
    let cmd =
      Cmd.v
        ...
        Term.(
          const grep $ Args.pattern $ Args.files $ Args.color $ Args.strings
          $ Args.regexp )
    in
    Cmd.eval_value cmd
  with
