...
module Stream = struct
  ...
  let next stream =
    let shift stream tok region =
      ...
      ; last = tok
      ; before_last =
          (match stream.last with COMMENT -> stream.before_last | tok -> tok)
      }
    in
    match Nstream.next stream.nstream with
    | Some ({ Nstream.token; region; _ }, nstream) ->
      if stream.stop (Pos.Region.snd region) then (EOF, shift stream EOF region)
      else (token, shift { stream with nstream } token region)
    | _ -> (EOF, shift stream EOF stream.region)

  let equals st1 st2 = st1.nstream == st2.nstream
...
let close_def stream =
  match Stream.previous stream with
...
let rec skip_to_next_paren stream =
  ...
  match tok with RPAREN | EOF -> stream | _ -> skip_to_next_paren stream
...
let rec close t scope =
  match t with
  | [] -> []
  | (scope1, _) :: r when scope1 = scope -> r
  | _ :: r -> close r scope

let maybe_close t scope =
  match t with (scope1, _) :: r when scope1 = scope -> r | t -> t
...
let parse_functor_args stream =
  let rec aux stream =
    ...
    | LPAREN, UIDENT x, COLON, stream -> begin
...
let parse t stream0 =
  ...
  | LBRACE -> (
    if stream0.last = INFIXOP3 "%" then (t, stream)
    else (* for mly headers *)
      match parse_path stream with
  ...
  | MODULE ->
    ...
    let ident, stream =
      ...
      | TYPE, stream1 -> (
        match Stream.next stream1 with
        | UIDENT u, stream -> (u, stream)
        | _ -> ("", stream) )
      | _ -> ("", stream)
    in
...
let read_string string = read_nstream (Nstream.of_string string)
...
let to_list =
  let aux acc t =
    List.fold_left
      ...
      acc t
  in
  function (Brace, ctx) :: t -> aux (List.rev ctx) t | t -> aux [] t

let fold_nstream f acc ?(init = []) ?stop nstream =
  let rec aux acc t stream =
    if Stream.previous stream = EOF then acc
    else
