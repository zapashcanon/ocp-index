...
let option_iter opt f = match opt with Some x -> f x | None -> ()
...
module IndexFormat = struct
  let list ?(paren = false) ?(left = fun _ -> ()) ?(right = fun _ -> ()) pr sep
    fmt lst =
  ...
  type coloriser =
    { f :
        'a.
        kind -> ('a, Format.formatter, unit) format -> Format.formatter -> 'a
    }
  ...
  let name ?(colorise = no_color) fmt id = colorise.f id.kind "%s" fmt id.name
  ...
  let rec tydecl fmt =
    ...
    | Otyp_record fields ->
      ...
      Format.fprintf fmt "@[<hv 2>{%a}@]"
        ...
        fields
    | Otyp_sum [] -> Format.pp_print_char fmt '-'
    | Otyp_sum constrs ->
      let print_variant fmt
        { Outcometree.ocstr_name = name
        ; ocstr_args = tyl
        ; ocstr_return_type = ret_type_opt
        } =
        match ret_type_opt with
        | None ->
          if tyl = [] then Format.pp_print_string fmt name
          else
            Format.fprintf fmt "@[<2>%s of@ @[%a@]@]" name
              (list !Oprint.out_type (fun fmt () -> Format.fprintf fmt " *@ "))
              tyl
        | Some ret_type ->
          if tyl = [] then
            Format.fprintf fmt "@[<2>%s :@ @[%a@]@]" name !Oprint.out_type
              ret_type
          else
            Format.fprintf fmt "@[<2>%s :@ @[%a -> @[%a@]@]@]" name
              (list !Oprint.out_type (fun fmt () -> Format.fprintf fmt " *@ "))
              tyl !Oprint.out_type ret_type
      in
      list print_variant
        ~left:(fun fmt ->
        ...
        fmt constrs
    | ty -> !Oprint.out_type fmt ty

  let out_ty fmt ty =
    let open Outcometree in
    match ty with
    | Osig_class (_, _, _, ctyp, _) | Osig_class_type (_, _, _, ctyp, _) ->
      !Oprint.out_class_type fmt ctyp
    | Osig_typext ({ oext_args = []; _ }, _) -> Format.pp_print_char fmt '-'
    | Osig_typext ({ oext_args; _ }, _) ->
      list ~paren:true !Oprint.out_type
        (fun fmt () ->
          Format.pp_print_char fmt ',';
          Format.pp_print_space fmt () )
        fmt oext_args
    | Osig_modtype (_, mtyp) | Osig_module (_, mtyp, _) ->
      !Oprint.out_module_type fmt mtyp
    | Osig_type ({ otype_type; _ }, _) -> tydecl fmt otype_type
    | Osig_value { oval_type; _ } -> !Oprint.out_type fmt oval_type
    | Osig_ellipsis -> Format.fprintf fmt "..."

  let ty ?(colorise = no_color) fmt id =
    option_iter id.ty (colorise.f Type "@[<hv>%a@]" fmt out_ty)

  let parent_ty ?colorise ?short fmt id =
    option_iter (IndexMisc.parent_type id) (fun id ->
      Format.fprintf fmt "@[<hv>%a =@ %a@]" (path ?colorise ?short) id
        (ty ?colorise) id )
  ...
  let loc ?root ?(intf = false) ?colorise:(_ = no_color) fmt id =
    let loc = if intf then Lazy.force id.loc_sig else Lazy.force id.loc_impl in
    if loc = Location.none then
      ...
      let fname =
        ...
        | Some r ->
          ...
          if Filename.is_relative fname then Filename.concat r fname
          else
      ...
      in
      Format.fprintf fmt "@[<h>%s:%d:%d@]" fname pos.Lexing.pos_lnum
  ...
  let info ?(colorise = no_color) fmt id =
    ...
    Format.fprintf fmt "@[<v 2>@[<hov 2>%a@ %a%a%a@]%a%a@]@."
      (path ?short:None ~colorise)
      id (kind ~colorise) id (breakif 0) id.ty (ty ~colorise) id (breakif 2)
      (Lazy.force id.doc)
      (doc ?escaped:None ~colorise)
      id

  let handle_format_char ?root chr ?colorise fmt id =
    match chr with
  ...
  let format ?root ?(separate = false) format ?colorise fmt id =
    ...
    let rec aux addsub ffmt flush i =
      ...
      if j >= len - 1 then addsub j (len - j)
        ...
        let fmt = ffmt () in
        begin
          match (format.[j], format.[j + 1]) with
...
module Print = struct
  ...
  let make (f : ?colorise:IndexFormat.coloriser -> 'a) ?(color = false) id =
    let colorise = if color then IndexFormat.color else IndexFormat.no_color in
