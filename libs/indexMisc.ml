...
let debug =
  if debug_enabled then fun fmt ->
    Printf.eprintf ("\027[36m" ^^ fmt ^^ "\027[m%!")
...
let key_to_string l =
  let rec aux n = function
    ...
    | c :: r ->
      let s = aux (n + 1) r in
      Bytes.set s n (if c = dot then '.' else c);
      s
  in
  let s = aux 0 l in
  Bytes.to_string s
...
let parent_type id =
  ...
  | Type | Value | Exception | Module | ModuleType | Class | OpenType
  | ClassType | Keyword ->
    None
...
let find_build_dir path =
  ...
  | Some ("_build" as dir) ->
    if Array.mem "default" (Sys.readdir (path / dir)) then
      Some (path / dir / "default")
    else Some (path / dir)
  | Some _ -> Some path
...
let make_relative ?(path = Sys.getcwd ()) file =
  if Filename.is_relative file then file
    ...
    let rec aux cwd file =
      match (cwd, file) with
      | d1 :: cwd, d2 :: file when d1 = d2 -> aux cwd file
      | cwd, file ->
        List.fold_left (fun file _ -> Filename.parent_dir_name :: file) file cwd
    in
  ...
  let rec find path =
    if path = home then None
      ...
      | None ->
        ...
        if path = parent then None else find parent
